...
public class VolunteerService {
    ...
    // throw new IllegalArgumentException("At least one volunteering type is
    // required.");
    // }
    // if (volunteerProfileDto.getSkills() == null ||
    // volunteerProfileDto.getSkills().isEmpty()) {
    // throw new IllegalArgumentException("At least one skill is required.");
    // }
    // if (volunteerProfileDto.getAbout() == null ||
    // volunteerProfileDto.getAbout().trim().isEmpty()) {
    ...
    // throw new WrongAuthenticationCredentials("User not authenticated. Please log
    // in first.");
    // }
    ...
    // if (user.getName() == null || user.getPhone() == null || user.getArea() ==
    // null) {
    // throw new IllegalArgumentException("User details are incomplete. Please
    ...
    public void addVolunteer(VolunteerFormDto volunteerProfileDto) {
        ...
        if (hospitalRole == null) {
            throw new IllegalArgumentException("Role not found: HOSPITAL");
        }

        // If the user already has the VOLUNTEER role, they have already been verified
        if (user.getRoles().contains(volunteerRole)) {
            throw new VolunteerAccessed("User already has the VOLUNTEER role.");
        }
        // If the user has HELP_REQUESTER or HOSPITAL roles, they cannot apply as a
        // volunteer
        if (user.getRoles().contains(helpRequesterRole)) {
            throw new HelpRequestorAccessed("User already has the HELP_REQUESTER role and cannot be a volunteer.");
        }
        if (user.getRoles().contains(hospitalRole)) {
            throw new IllegalArgumentException("User already has the HOSPITAL role and cannot be a volunteer.");
        }

        // Ensure user's profile details are complete
        ...
        // Create a new Volunteer entity with verified=false (pending verification)
        Volunteer volunteer = new Volunteer();
        volunteer.setName(user.getName());
        volunteer.setUser(user);
        volunteer.setPhone(user.getPhone());
        ...
        // Save the volunteer request to the database
        volunteerRepository.save(volunteer);

        // Optionally notify the manager of the new volunteer request (e.g., via email)
    ...
    // Volunteer volunteer = volunteerRepository.findByUser(user)
    // .orElseThrow(() -> new UsersNotFound("Volunteer not found with email: " +
    // email));
